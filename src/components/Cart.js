...
const Cart = () => {
    ...
    const totalAmount = carroVacio? 0 : carrito.map((e) => e.cantidad * e.detalle.price).reduce((a, b) => a + b);
    const totalQuantity = carroVacio? 0 : carrito.map((e) => e.cantidad).reduce((a, b) => a + b)

    if (!carroVacio) {
        return (
            <div className="container">
                <h2>Carrito de Compras</h2>
                <h3>{carrito.length} Productos | {totalQuantity} Unidades en Total </h3>
